...
server <- function(input, output, session) {
  output$staticPlot <- renderPlotly({
    ...
    plt <- ggplot(continent_combined, aes_string(x = input$xcol, y = input$ycol, size = input$size, color = "Continent", text="tooltip_content")) +
    ...
    observe({
      brush <- event_data("plotly_selected", source = "staticPlot")
      if (!is.null(brush)) {
        brushed_points <- brush[["points"]]
        if (length(brushed_points) > 0) {
          plt$x$data[[1]]$marker$line$color <- rep('rgba(0,0,0,0)', nrow(continent_combined))
          plt$x$data[[1]]$marker$line$color[brushed_points$pointNumber + 1] <- 'transparent'
        }
      }
    })
